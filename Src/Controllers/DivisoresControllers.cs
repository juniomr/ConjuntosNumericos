...
namespace ConjuntosNumericos.Controllers;
...
public class DivisoresController : Controller
{
    ...
    [HttpGet]
    public IActionResult Calcula([FromQuery] long numeroEntrada)
    {
        try
        {
    ...
    [HttpGet]
    public IActionResult CalculaPrimos([FromQuery] long numeroEntrada)
    {
        try
        {
            if (ValidarIntervalo(numeroEntrada))
